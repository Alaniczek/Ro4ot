...
//        $this->commandManager = null;
    ...
    public function forcePingToAllRobots(string $ip, string $port): void
    {
        $Robots = $this->getRobotUnits();
        $logger = Logger::getInstance();
        $pingCommand = "I " . $ip . " " . $port . "X";

        foreach ($Robots as $units) {
            $cmdManager = new CommandManager($units['IP'], (int)$units['Port'], $logger);
            $logger->log("Wysłano ping do robota: " . $units['Name'] . " (" . $units['IP'] . ":" . $units['Port'] . ")");
            $cmdManager->sendCommand($pingCommand);
        }
    }
}